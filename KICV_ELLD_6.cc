...
polynomeModulo polynomeModulo :: quotient		(const polynomeModulo &rightHandSide) const {
	...
	unsigned int currentExponent(result.att_value.size() - 1); 	/* To remember where we're at */
	while (localCopy) {
...
polynomeModulo polynomeModulo :: modInv			() const {
	...
	if (polynomeModulo :: extendedEuclidean(polynomeModulo(att_modulo), polynomeModulo(att_value), NULL, &inverse, false) != 1) {
